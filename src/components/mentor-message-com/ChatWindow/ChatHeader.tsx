...
interface ChatHeaderProps {
  ...
  onRoomImageClick?: () => void;   // ← clicking group avatar opens RoomImageModal
}
...
const Avatar: FC<{
  ...
  return (
    ...
    >
      ...
      {/* Edit overlay — only for group avatar */}
      {editable && (
...
const ChatHeader: FC<ChatHeaderProps> = ({ chat, onBack, showBackButton, onRoomImageClick }) => {
    ...
    <div className="flex items-center gap-3 px-5 py-3.5 bg-white border-b border-gray-100 shadow-sm">
      ...
      {showBackButton && onBack && (
        <button onClick={onBack}
          className="lg:hidden flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100 transition">
      ...
      {/* Avatar — group avatar is clickable to change image */}
      <Avatar
        src={chat.userAvatar}
        name={chat.userName}
        size="w-10 h-10"
      ...
      <div className="flex-1 min-w-0">
        ...
        {isGroup ? (
          <p className="text-xs text-gray-400 truncate">
            {chat.mentorshipId ? `Mentorship #${chat.mentorshipId}` : 'Group Chat'}
          </p>
        ) : (
      ...
      <div className="flex items-center gap-1">
        {/* Change photo button — group only */}
