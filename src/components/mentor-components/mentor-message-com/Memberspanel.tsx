...
const MembersPanel: FC<Props> = ({
  roomId, roomName, creatorEmail, myEmail, onClose, onDeleteRoom,
}) => {
    ...
    <div className="absolute inset-y-0 right-0 w-72 bg-white border-l border-gray-100 shadow-xl flex flex-col z-20">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        {!loading && !error && members.map(m => {
          ...
          // ← use userImageUrl (actual API field)
          const src = m.userImageUrl
            ? (m.userImageUrl.startsWith('http') ? m.userImageUrl : `${BASE_URL}${m.userImageUrl}`)
            : null;

          return (
            <div key={m.userId ?? m.email}
              ...
              {/* Role */}
              {m.role && (
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${roleColor(m.role)}`}>
                  {m.role}
                </span>
              )}
            </div>
          ...
          <>
            {!confirmDel ? (
              ...
              <div className="space-y-1.5">
                <p className="text-xs text-red-500 text-center font-medium">Delete this group? This can't be undone.</p>
