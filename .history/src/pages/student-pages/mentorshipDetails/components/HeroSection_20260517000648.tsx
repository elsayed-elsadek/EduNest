...
const HeroSection: FC<HeroSectionProps> = ({ mentorship, mentorshipId, isEnrolled }) => {
  ...
  const handleContactMentor = () => {
    if (!mentorship?.mentorEmail) return;
    navigate('/student/messages', {
      state: {
        openDirectChatWith: {
          email:  mentorship.mentorEmail,
          name:   mentorship.mentorName,
          avatar: mentorship.mentorProfileImageUrl
            ? `${API_BASE_URL}${mentorship.mentorProfileImageUrl}`
            : undefined,
        },
      },
    });
  ...
  return (
    <section
      ...
      style={{
backgroundImage:mentorship?.coverImageUrl && !imageError
    ? `url(${API_BASE_URL}${mentorship.coverImageUrl})`
...
=======
        <div className={`grid grid-cols-1 ${!isEnrolled ? 'lg:grid-cols-[1.25fr_0.75fr]' : ''} gap-8 lg:gap-12 items-center`}>
>>>>>>> Student-role
              ...
              <div className="flex flex-col gap-5 lg:gap-6">
                ...
                <div className="grid grid-cols-1 gap-3 lg:gap-4">
                  ...
                  <button
                    ...
                    disabled={!mentorship?.mentorEmail}
                    className="w-full px-6 lg:px-8 py-4 lg:py-5 bg-slate-100 text-slate-700 rounded-xl lg:rounded-2xl font-bold text-base lg:text-lg hover:bg-slate-200 transition-all flex items-center justify-center gap-2 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    <MessageCircle className="w-4 h-4 lg:w-5 lg:h-5" />
                    Contact Mentor
                  </button>
